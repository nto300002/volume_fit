# Issue 002: dev/stg/prod 環境を分離する
...
development、staging、production をアプリ設定として明確に分離する。
...
- [ ] 環境ごとの設定を切り替えられる
- [ ] production 設定で誤って development に接続しない
- [ ] 環境表示は production に含めない
...
| No. | 操作 | 期待結果 | 結果 |
|---:|---|---|---|
| 1 | 各環境ビルドで接続先と表示が期待通りか確認する。 | 期待どおりに動作する | 未実施 |
